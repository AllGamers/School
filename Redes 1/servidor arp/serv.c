...
unsigned char ipDestino[4]= {0x08,0x08,0x00,0x3c};
...
void recibeARP(int, unsigned char *);
...
void estructuraTrama(unsigned char *trama){
	...
	memcpy(trama+14,"alan altamira",40);
}
...
void protege(int ds,int indice, unsigned char *trama, struct ips * myips){
	...
	unsigned char Opcode2[2]={0x00,0x02};
	unsigned char MAC0[6]={0x00,0x00,0x00,0x00,0x00,0x00};
        unsigned char IP0[4]={0x00,0x00,0x00,0x00};
	...
	while(mtime<1000){
		...
		if(tam == 1){
			...
			if((!memcmp(trama+12,arptype,2))&&(!memcmp(trama+20,Opcode,2))){
				aux = myips;
				while(aux != NULL){
					if(!memcmp(trama+28,IP0,4)){
						if(!memcmp(trama+38,aux->ip,4)){
							if(!memcmp(trama+6,aux->mac,6)){
								bandera = 1;
							}else{
								memcpy(MACOrigen,aux->mac,6);
								memcpy(ipOrigen,aux->ip,6);
				...
				if(bandera == 0){
					...
					memcpy(trama+32,MAC0,6);
					
					enviarTrama(ds,indice,trama);
					recibeARP(ds,trama);
					printf("se defendio la ip: ");
					for(i=0;i<4;i++){
						printf(" %d.",ipOrigen[i]);
					}
				}
...
void recibeARP(int ds, unsigned char *trama){
...
void doARP(int ds, int indice, unsigned char *trama){
    unsigned char hardware[2]={0x00,0x01};
    unsigned char protocolo[2]={0x08,0x00};
    unsigned char LHard[1]={0x06};
    unsigned char LProt[1]={0x04};
    ...
    printf("\nInsertar la IP Destino:  ");
	scanf("%s", nombre);
    
    inet_aton(nombre,(struct in_addr *)ipDestino);        
