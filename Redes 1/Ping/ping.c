...
void setIPHeader(unsigned char * trama, unsigned int protocol, unsigned char * sourceIPAddr, unsigned char *targetIPAddr, int count){
    ...
    memcpy(trama+14, "\x45", 1);
    memset(trama+17, PINGTOTLEN, 1);
    memset(trama+18, identifier[1], 1);
    memset(trama+19, identifier[0], 1);
    memcpy(trama+20, "\x40", 1);
    memset(trama+22, TTL, 1);
    memset(trama+23, protocol, 1);
    memset(trama+24, 0, 2);
    memcpy(trama+26, sourceIPAddr, 4);
    memcpy(trama+30, targetIPAddr, 4);
...
void setICMPHeader(unsigned char * trama, unsigned short seqNumber, int ID){
    unsigned char identifier[2];
    unsigned char checkSum[2];
    unsigned char data[31]={"Redes Quintanilla Israel Morales"};
    unsigned short ckSum;
    memcpy(trama+34, "\x08", 1);
    unsigned char * ptr, *ptr1;
    ptr =(char *) &ID;
    ptr1=ptr;
    for(&ptr; *ptr; ptr++){
    ...
    memset(trama+36, 0, 2);
    memset(trama+38, identifier[1], 1);
    memset(trama+39, identifier[0], 1);
    memset(trama+41, seqNumber, 1);
...
int filterICMPreply(unsigned char *trama, unsigned char * destHwAddr, unsigned char * sourceHwAddr, unsigned char * targetIPAddr, unsigned char * sourceIPAddr, int pID, unsigned short seqNumber, int tam_rcv_from, struct timeval *tval_now){
    ...
    if(!memcmp (trama + 12, ETHTYPE_ICMP, 2) && !memcmp (trama + 38, identifier, 2) && !memcmp (trama + 41, sqnum, 1)){
        ...
        printf("%d.%d.%d.%d : SECUENCIA ICMP=%d tiempo de vida=%d tiempo=%.2f", trama[26], trama[27], trama[28], trama[29], sqnum[0], trama[22], timeres);
        printf("\n");
        return 1;
    }
    return -1;
