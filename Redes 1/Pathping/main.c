...
{
    if(geteuid()!=0){
        printf("%ssudo su es necesario para poder ejecutar tu argumento\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    if(argc != 2)
        usage(argv);
    ...
    printf("\nRUTA %s (%s), %d con un maximo de saltos, tamano de paquete (%d) en bytes.\n\n" , hostname , ipstring_dest, MAXHOP,icmpdatabytes);
    ...
    //printf("\n");
    //print_list(cabeza);
    printf("\nObteniendo datos; envío de %d paquetes\n", nodostotales*100*2);
    unsigned int seq=1;
