...
void analizaLLC(unsigned char * tramaRecv){
  int i;
  //ENCABEZADO MAC
  printf("DIRECCION MC DESTIONO: ");
    printf("%.2x",tramaRecv[0]);
    for(i=1;i<6;i++){
      printf(":%.2x",tramaRecv[i]);
    }
    printf("\t");
  printf("DIRECCION MAC ORIGGEN ");
  ...
  if(crtl==0||crtl==2){
      ...
      printf("FINAL: %d ",tramaRecv[17]&0x01);
      
  }
  if(crtl==3){
    //No Numerada
    printf("Norma / Extendida\n");

    printf("tFINAL: %d ",(tramaRecv[16]&0x10)>>4);
    //Vemos el estado del POLL/FINAL
    if(((tramaRecv[16]&0x10)>>4)==1){
